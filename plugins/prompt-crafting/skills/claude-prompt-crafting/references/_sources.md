...
| 8 | https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-fable-5-1 | **New (2026-09-07): Fable 5.1 / Mythos 5.1** (current frontier, supersedes #4) — effort re-sweep (`low` now cost-competitive), fewer default progress updates, tool-call batching in coding/computer-use loops, append-only conversation history / thinking-block binding, writing density, formatting-in-chat defaults, targeted edits over whole-file rewrites, fewer safeguard false positives |
...
| `models/fable.md` (Fable 5.1 + Mythos 5.1, deltas from Fable 5 + Mythos 5) | **#8** — dedicated Fable 5.1 / Mythos 5.1 prompting page (current frontier); **#4** — prior-gen Fable 5 / Mythos 5 page, kept for the still-valid baseline guidance |
...
> Six prompting URLs are tracked in `.source-hashes.json` as of this note; the **Fable 5.1** page (#8) is
> **new this cycle (2026-09-07) and not yet seeded there** — this routine's instructions bar it from touching
> `.source-hashes.json` (the check-sources Action owns that file), so a maintainer or the next Action run needs
> to add `"https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-fable-5-1":
> null` to it. Earlier: the **Opus 5** page (#3) was added seeded `null` (the check-sources Action captured its
> baseline on a later run), and the retired **prompting-tools** page (#5) was removed from tracking (it now
> mirrors #2). The Haiku overview page is not tracked — it churns on pricing/availability; `models/haiku.md`'s
> substance comes from the cross-model best-practices page (#2). Add a new tracked URL only if Anthropic ships
> a dedicated Haiku page.

## Volatile items to recheck each cycle
- **Prefill removal** — applies to Claude 4.6 and newer, incl. the 5-series (Fable 5.1 / Mythos 5.1, Fable 5 /
  Mythos 5, Sonnet 5); confirm.
...
- **Per-model pages** — model names (Opus / Sonnet / Haiku / Fable / Mythos + versions) and their tips
  change; current set: **Fable 5.1, Mythos 5.1** (new current frontier, own dedicated page #8, superseding
  Fable 5 / Mythos 5 as of this cycle), Fable 5, Mythos 5 (prior gen, page #4 still valid baseline), **Opus 5**
  (flagship, `claude-opus-5`, own dedicated page), Sonnet 5, Haiku 4.5. **Opus 4.8 / 4.7 / 4.6 are now legacy**
  (the 4.8 page is still live, tracked as #7). Dedicated prompting pages exist for Fable 5.1, Opus 5, Sonnet 5,
  Fable 5 / Mythos 5, and legacy Opus 4.8; **no Haiku page** yet — recheck.
- **Refusal categories & fallback (Fable 5.1 / Mythos 5.1, Fable 5 / Mythos 5)** — `reasoning_extraction`
  (don't ask the model to reproduce its reasoning as text), offensive-cyber, bio/life-sciences; declined
  requests return `stop_reason: "refusal"`. Fable 5.1's page (#8) reconfirms the same categories and says its
  classifiers produce **fewer false positives** than Fable 5's did at launch (three residual triggers: compile-
  check phrasing, lesser-known languages, base64 in tool output) — but does **not** restate the Opus-4.8
  fallback target on-page (that lives on the untracked `whats-new-fable-5-1` page); treat the fallback target
  as unconfirmed for 5.1 specifically until a source page states it directly.
- **Fable 5.1 conversation-history / thinking-block binding (new)** — for accounts created on or after
  2026-08-31, a replayed thinking block whose prefix (system prompt, tool list, or an earlier message) changed
  returns a 400, or is dropped with `thinking.block_binding.prefix_mismatch_behavior: "drop_block"` (beta).
  Practical effect: don't edit earlier turns, rewrite `system`/`tools`, or summarize in place mid-conversation;
  use turn-scoped system messages / mid-conversation system messages / server-side compaction instead. Expected
  to become universal on future models — recheck whether it's since started applying to all accounts.
