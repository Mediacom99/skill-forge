...
- [x] ~~**Clean up the clipboard scratch file after a successful copy**~~ · **solved in 0.4.2 (no file at all)**
  - Final approach: clipboard delivery pipes the prompt via a here-doc (`pbcopy << 'EOF' … EOF`) on stdin —
    **no file is ever written**, so there's nothing to clean up or leave behind, and the skill stays fully
    read-only (no `rm`, no `allowed-tools` change). A file-based fallback remains for shells without here-doc
    support. (0.4.1's write-to-cwd + empty-after was the interim step; superseded.)
